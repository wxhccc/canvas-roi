...
function setCtxStyles(this: any): void {
...
function clearExistRoiPath(this: any) {
  this.$ctx.clip();
  this.clearCanvas();
}

function drawNewRoiPath(this: any, path: RoiPath, movePoint?: Point) {
  if (!path) return;
  const { type } = path;
...
function drawRoiPaths(this: any, movePoint?: Point, notClear?: boolean): void {
...
function drawPointSign(this: any, points: Point[]) {
  this.$ctx.save();
...
function drawOpeDragPoint(this: any, point: Point) {
  this.$ctx.save();
  this.$ctx.beginPath();
  const {
...
function drawRoiPathsWithOpe(this: any, circlePoint: Point): void {
  this._drawRoiPaths();
