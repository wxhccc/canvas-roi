...
export function jsonClone<T = any>(value: T): T {
...
export function getVirtualRectPoints(points: Point[]) {
  return points.length === 2 ? [points[0], { x: points[1].x, y: points[0].y }, points[1], { x: points[0].x, y: points[1].y }] : points;
}

export function countDistance(point1: Point, point2: Point) {
  return Math.sqrt((point1.x - point2.x) ** 2 + (point1.y - point2.y) ** 2);
}

export function checkPointsEqual(oPoint: Point, dPoint: Point) {
...
export function bindMethods(methods: MethodsMap) {
